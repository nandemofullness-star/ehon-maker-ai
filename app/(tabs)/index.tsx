...
import { usePremium, FREE_AI_USES_PER_CYCLE } from "@/hooks/use-premium";
...
export default function HomeScreen() {
  ...
  const remakePageById = useCallback(
    async (pageId: string) => {
      if (isBatchProcessing || isGeneratingPdf) return;
      // Ad-only limit check: consume one AI use; if 0 remaining, reward ad modal is shown
      const allowed = await premium.consumeAiUse();
      if (!allowed) {
        // showRewardAd is now true — modal will appear automatically
        return;
      }
  ...
  const batchRemakeWithAI = useCallback(async () => {
    ...
    // Ad-only limit check: consume one AI use for the batch
    const allowed = await premium.consumeAiUse();
    if (!allowed) {
      // showRewardAd is now true — modal will appear automatically
      return;
  ...
  // AI uses remaining badge in header
  const AiUsesBadge = (
    <View style={styles.premiumBadge}>
      <MaterialIcons name="auto-awesome" size={12} color="#6366f1" />
      <Text style={styles.premiumBadgeText}>AI残り{premium.aiUsesRemaining}回</Text>
    </View>
  );
    ...
    <ScreenContainer containerClassName="bg-background">
      ...
      <DraggableList
        ...
        ListHeaderComponent={
          ...
          {/* AI uses remaining info banner */}
          {pages.length > 0 && premium.aiUsesRemaining <= 1 && (
            <View style={[styles.warningBanner, { backgroundColor: "#EEF2FF", borderColor: "#6366f1" }]}>
              ...
              <View style={styles.warningText}>
                <Text style={[styles.warningTitle, { color: "#6366f1" }]}>
                  AI変換残り{premium.aiUsesRemaining}回
                </Text>
                <Text style={[styles.warningBody, { color: "#4338ca" }]}>
                  使い切ると広告を視聴してプラス{FREE_AI_USES_PER_CYCLE}回追加できます。
                </Text>
              </View>
            </View>
          )}
      ...
      {/* Ad Banner (always shown) */}
      <AdBanner />

      {/* Reward Ad Modal — shown when AI uses run out */}
      <RewardAdModal
        visible={premium.showRewardAd}
        onAdWatched={premium.onAdWatched}
        onDismiss={premium.dismissAdModal}
      />
